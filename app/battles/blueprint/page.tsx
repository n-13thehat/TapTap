...
import { Swords, Sparkles, Youtube, Coins, MessageSquareMore, Lock, Gauge, Flame, Search, Settings, Bell } from "lucide-react";
...
function Hero() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-fuchsia-500/10 to-rose-400/10">
      ...
      <div className="relative z-10 p-6 md:p-10">
        ...
        <h1 className="bg-gradient-to-r from-teal-200 via-white to-teal-100 bg-clip-text text-2xl font-bold tracking-tight text-transparent md:text-4xl">
          Battles — Competitive Artistry, Entertainment Economy
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-white/80">Competitor DNA: YouTube × Patreon × Verzuz. Purpose: Competition meets community and TapCoin.</p>
      </div>
    </div>
  );
}
...
function Features() {
  return (
    <section>
      <SectionHeader icon={Swords} title="Features" subtitle="UX & business logic" />
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <div className="mb-2 text-xs text-white/60">Battle Cards</div>
          <div className="text-sm text-white">YouTube‑style grid of matchups; thumbnails show artists’ stats + cost to unlock.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Paywall</div>
          <div className="text-sm text-white">Pay‑per‑view: 2–50 TapCoin entry depending on hype/demand.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Weekly Battle Event</div>
          <div className="text-sm text-white">Community poll chooses featured matchup. Streams free to all, sponsored by Treasury pool.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Engagement</div>
          <div className="text-sm text-white">Comments, tips, live chat; viewer reactions ripple across background.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Design Language</div>
          <div className="text-sm text-white">Gritty neon overlays; responsive to audience hype level.</div>
        </Card>
      </div>
    </section>
  );
...
export default function BattlesBlueprintPage() {
  return (
