...
import { Radio, Sparkles, MessageCircle, HandCoins, CalendarClock, BarChart4, Search, Settings, Bell, Video, Image as ImageIcon } from "lucide-react";
...
function Hero() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-emerald-500/10 to-cyan-400/10">
      ...
      <div className="relative z-10 p-6 md:p-10">
        <div className="mb-3 flex items-center gap-2">
          <Badge>Live</Badge>
          <Pill>Performance Energy</Pill>
          <Pill>Culture in Real Time</Pill>
        </div>
        <h1 className="bg-gradient-to-r from-teal-200 via-white to-teal-100 bg-clip-text text-2xl font-bold tracking-tight text-transparent md:text-4xl">
          Live — Real-time Broadcast Hub
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-white/80">Competitor DNA: Twitch × TikTok Live. Purpose: Connection and performance.</p>
      </div>
    </div>
  );
}
...
function Modes() {
  return (
    <section>
      <SectionHeader icon={Radio} title="Modes" subtitle="UX approaches" />
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <div className="mb-2 text-xs text-white/60">Performance Live</div>
          <div className="text-sm text-white">Landscape video + chat sidebar (Twitch layout). Tips appear as floating TapCoin bursts (0% tax).</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Lifestyle Live</div>
          <div className="text-sm text-white">Vertical full‑screen, TikTok feel; emoji reaction bubbles and quick follow.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Creator Panel</div>
          <div className="text-sm text-white">Setup wizard: stream key, title, schedule, and tip options.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Community Polls</div>
          <div className="text-sm text-white">Audience votes on featured battles and weekly showcases.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Discovery</div>
          <div className="text-sm text-white">Live carousel with colored frames by genre and mood.</div>
        </Card>
      </div>
    </section>
  );
...
export default function LiveBlueprintPage() {
  return (
