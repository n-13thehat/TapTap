...
import { Cpu, Sparkles, QrCode, Link2, Fingerprint, Radio, ShieldCheck, Waves, Search, Settings, Bell } from "lucide-react";
...
function Hero() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-teal-500/10 to-sky-400/10">
      ...
      <div className="relative z-10 p-6 md:p-10">
        <div className="mb-3 flex items-center gap-2">
          ...
          <Pill>Matrix‑Core Terminal</Pill>
        </div>
        <h1 className="bg-gradient-to-r from-teal-200 via-white to-teal-100 bg-clip-text text-2xl font-bold tracking-tight text-transparent md:text-4xl">
          Mainframe — Devices, Identity, and Power
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-white/80">Theme: Matrix‑core visual terminal for linking devices and verifying identity.</p>
      </div>
    </div>
  );
}
...
function Features() {
  return (
    <section>
      <SectionHeader icon={Cpu} title="Features" subtitle="Security and linking" />
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <div className="mb-2 text-xs text-white/60">NFC Linking</div>
          <div className="text-sm text-white">Link devices with NFC tap; ring illuminates when chip or phone connects.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">QR Login</div>
          <div className="text-sm text-white">Scan to authenticate securely across devices.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Bio Verification</div>
          <div className="text-sm text-white">Fingerprint/face confirmation for sensitive operations.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Audio</div>
          <div className="text-sm text-white">Sub‑bass thrum symbolizes power‑on state.</div>
        </Card>
      </div>
    </section>
  );
...
export default function MainframeBlueprintPage() {
  return (
