...
import { Waves, Sparkles, Music4, PlaySquare, HandCoins, ListMusic, Search, Settings, Bell, Minimize2 } from "lucide-react";
...
function Hero() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-indigo-500/10 to-teal-400/10">
      ...
      <div className="relative z-10 p-6 md:p-10">
        <div className="mb-3 flex items-center gap-2">
          <Badge>Surf</Badge>
          <Pill>Infinite Discovery</Pill>
          <Pill>Cosmic Flow</Pill>
        </div>
        <h1 className="bg-gradient-to-r from-teal-200 via-white to-teal-100 bg-clip-text text-2xl font-bold tracking-tight text-transparent md:text-4xl">
          Surf — Continuous Playback Discovery
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-white/80">Competitor DNA: Musi × YouTube Mini‑Player. Purpose: Continuous audio/video discovery.</p>
      </div>
    </div>
  );
}
...
function Features() {
  return (
    <section>
      <SectionHeader icon={Waves} title="Features" subtitle="TapTap UX concepts" />
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <div className="mb-2 text-xs text-white/60">Feed</div>
          <div className="text-sm text-white">Streamlined catalog of trending songs and music videos.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Mini‑Player</div>
          <div className="text-sm text-white">Shrinkable floating video window (YouTube‑like PiP). Keeps playing while browsing.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Background Play</div>
          <div className="text-sm text-white">Auto‑minimizes to corner; audio continues.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Tipping</div>
          <div className="text-sm text-white">Inline tips with no tax.</div>
        </Card>
        <Card>
          <div className="mb-2 text-xs text-white/60">Personalization</div>
          <div className="text-sm text-white">Curated playlists based on AstroProfile and listening energy.</div>
        </Card>
      </div>
    </section>
  );
...
export default function SurfBlueprintPage() {
  return (
