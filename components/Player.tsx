...
import { Play, Pause, SkipBack, SkipForward, List, Shuffle, Repeat, Volume2, VolumeX, BookText, PlusSquare, MoreHorizontal, MessageSquare, Sparkles, Activity } from "lucide-react";
...
export default function Player() {
  ...
  }, []);// Single-instance guard with hydration safety
  const [enabled, setEnabled] = useState(true);
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Use a more specific property to avoid conflicts
    const mountedKey = '__taptap_global_player_mounted';
    if ((window as any)[mountedKey]) {
      setEnabled(false);
      return;
    }
    (window as any)[mountedKey] = true;

    return () => {
      if ((window as any)[mountedKey]) {
        // Keep mounted across route transitions; only clear if truly unmounting last instance
        (window as any)[mountedKey] = false;
      }
    };
  }, []);
  ...
  // Fallback: respond to global event dispatches
  useEffect(() => {
    function onPlay(e: any) {
      ...
      audio.play().then(()=>{ try { recordEvent("TRACK_PLAYED", { id: track.id }); } catch {} }).catch(() => {});
    }
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-black/70 backdrop-blur-md border-t border-white/10 p-4 flex flex-col gap-4 sm:flex-row sm:items-center">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={()=>{ /* Placeholder save */ try{ recordEvent('TRACK_SAVED', { id: current?.id }); } catch{} }} className="text-white/70 hover:text-white" aria-label="Save">
          â¤
        </button>
        <button onClick={()=>{ /* Placeholder add to playlist */ try{ recordEvent('PLAYLIST_CREATED', { quickAdd: true, trackId: current?.id }); } catch{} }} className="text-white/70 hover:text-white" aria-label="Add to Playlist">
          <PlusSquare size={16} />
        </button>
      </div>
      ...
      {showQueue && (
        <div className="absolute bottom-14 right-4 z-50 w-80 max-h-72 overflow-y-auto rounded-xl border border-white/10 bg-black/90 backdrop-blur p-2 space-y-1">
          {Array.isArray(queue) && queue.length > 0 ? queue.map((t: any) => (
            <button key={t.id} onClick={()=>playTrack(t)} className={`w-full flex items-center gap-2 rounded-md px-2 py-1.5 text-left ${current?.id===t.id? 'bg-teal-500/20 text-teal-200' : 'bg-white/5 text-white/80 hover:bg-white/10'}`}>
